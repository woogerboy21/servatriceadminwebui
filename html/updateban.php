				...
				<td colspan="2" align="center">
					<?php
						if ($_SERVER['REQUEST_METHOD'] == 'POST'){
							if (empty($_POST['minutes'])){ echo "failed, minutes can not be blank"; exit; }
							...
							$dbserv = get_config_value($configfile,"dbserver");
                        			        ...
                        			        $query = mysql_query("UPDATE " . trim($dbtable) . " SET minutes='" . trim($_POST['minutes']) . "',reason='" . trim(mysql_real_escape_string($_POST['reason'])) . "',visible_reason='" . trim(mysql_real_escape_string($_POST['visiblereason'])) . "' WHERE user_name='" . trim($_POST['username']) . "' AND time_from='" . trim($_POST['starttime']) . "'");
